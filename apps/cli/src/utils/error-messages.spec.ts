...
describe('ErrorMessages', () => {
  describe('Configuration Errors', () => {
    ...
    it('should provide config parse failed message', () => {
      ...
      expect(ErrorMessages.CONFIG_PARSE_FAILED(error)).toBe(`❌ Failed to parse configuration file: ${error}`);
    });
  });
  ...
  describe('Operation Errors', () => {
    ...
    it('should provide operation failed message with reason', () => {
      expect(ErrorMessages.OPERATION_FAILED('Build', 'TypeScript errors')).toBe('❌ Build failed: TypeScript errors');
    });
  });

  describe('Resource Errors', () => {
    it('should provide resource not found message', () => {
      expect(ErrorMessages.RESOURCE_NOT_FOUND('app.button.ok')).toBe('❌ Resource key "app.button.ok" not found.');
    });

    it('should provide resource exists message', () => {
      expect(ErrorMessages.RESOURCE_EXISTS('app.button.ok')).toBe('❌ Resource key "app.button.ok" already exists.');
    });
  ...
  describe('Locale Errors', () => {
    it('should provide locale not found message', () => {
      expect(ErrorMessages.LOCALE_NOT_FOUND('fr')).toBe('❌ Locale "fr" not found in configuration.');
    });
  ...
  describe('File System Errors', () => {
    it('should provide file not found message', () => {
      expect(ErrorMessages.FILE_NOT_FOUND('/path/to/file.json')).toBe('❌ File not found: /path/to/file.json');
    });

    it('should provide directory not found message', () => {
      expect(ErrorMessages.DIRECTORY_NOT_FOUND('/path/to/dir')).toBe('❌ Directory not found: /path/to/dir');
    });

    it('should provide file read failed message without reason', () => {
      expect(ErrorMessages.FILE_READ_FAILED('/path/to/file.json')).toBe('❌ Failed to read file: /path/to/file.json');
    });
    ...
    it('should provide file write failed message without reason', () => {
      expect(ErrorMessages.FILE_WRITE_FAILED('/path/to/file.json')).toBe('❌ Failed to write file: /path/to/file.json');
    });
